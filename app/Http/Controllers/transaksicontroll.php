...
class transaksicontroll extends Controller
{
    ...
    //    //Ini adalah Lokasi printer dan prinnter yang digunakan
    //     $ip = '192.168.1.40';//IP Kompuuter kita atau priner lain yang masih satu jaringan
    //     $printer = 'POS 5';// Nama Printer yang disharing
    ...
    function pengembalian(Request $req){
        if(count($req->all())==0){
            ...
            from tb_peminjaman,tb_buku,tb_anggota,tb_koleksi_buku WHERE tb_peminjaman.no_anggota = tb_anggota.no_anggota AND tb_peminjaman.no_induk_buku =tb_koleksi_buku.no_induk_buku AND tb_koleksi_buku.kd_buku=tb_buku.kd_buku AND tb_peminjaman.status=1 AND tb_peminjaman.no_pinjam='".$req->get('no_pinjam')."'");
        }
